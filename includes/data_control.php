...
$connection= mysqli_connect("localhost","root","","admin_distances");
if(@$_POST['formName']=='insert_location'){
    ...
    if($updateStatus==0){
        mysqli_query($connection,"INSERT INTO `locations`(`loc_org_name`,`loc_sub_urb`,`loc_address`,`loc_facility`,`loc_facility_type`,`loc_latitude`,`loc_longitude`) values('$org_name','$sub_urb','$address','$facility',$facility_type,'$latitude','$longitude')");
        ...
        if(mysqli_query($connection,"UPDATE `locations` SET `loc_org_name`='$org_name',`loc_facility_type`=$facility_type,`loc_sub_urb`= '$sub_urb',`loc_address`='$address',`loc_facility`='$facility',`loc_latitude`='$latitude',`loc_longitude`='$longitude' WHERE loc_id=$updateStatus")){
            echo 1;
        }else{
            echo 0;
        }
...
if(@$_POST['formName']==''){
    ...
    while($locationQryRes=mysqli_fetch_array($locationQry)){
            ...
            if($locationQryRes['loc_facility_type']==1){                
                $facility_type='<span id="1">Aged Care High</span>';
            }else if($locationQryRes['loc_facility_type']==2){                
                $facility_type='<span id="2">Aged Care Low</span>';
            }else{
                $facility_type='<span id="3">Disability</span>';
            }
        ...
        $view='<button data="'.$locationQryRes["loc_id"].'" type="button" class="update_location btn btn-outline-primary btn-sm">Edit</button><button onclick="delete_loc('.$locationQryRes["loc_id"].')" type="button" class="delete_location btn btn-outline-danger btn-sm">Delete</button>';

        array_push($locations['data'],array('org_name'=>$locationQryRes['loc_org_name'], 'sub_urb'=>$locationQryRes['loc_sub_urb'],'address'=>$locationQryRes['loc_address'],'facility'=>$locationQryRes['loc_facility'],'facility_type'=>$facility_type,'latitude'=>$locationQryRes['loc_latitude'],'longitude'=>$locationQryRes['loc_longitude'],'option'=>$switch,'action_btn'=>$view));
